...
- Background video generation via Azure Sora (or similar generative AI)
...
3. Generates relevant background videos using Azure Sora based on search terms/prompts
...
- Background videos are generated by Azure Sora (or a similar generative AI service). Quality and relevance depend on the service and prompts.
- Generative video services may have costs, rate limits, and specific terms of use.

# General Requirements

- Internet connection
- Azure Sora API Key and Endpoint (or equivalent for other generative video services)
...
2. Search terms: Keywords used to generate a prompt for Azure Sora (or a similar service) to create a background video for the scene.
...
```jsx
docker run -it --rm --name short-video-maker -p 3123:3123 -e LOG_LEVEL=debug -e SORA_API_KEY=<YOUR_SORA_API_KEY> -e SORA_API_ENDPOINT=<YOUR_SORA_API_ENDPOINT> gyoridavid/short-video-maker:latest-tiny
```
...
```jsx
docker run -it --rm --name short-video-maker -p 3123:3123 -e LOG_LEVEL=debug -e SORA_API_KEY=<YOUR_SORA_API_KEY> -e SORA_API_ENDPOINT=<YOUR_SORA_API_ENDPOINT> gyoridavid/short-video-maker:latest
```
...
```jsx
docker run -it --rm --name short-video-maker -p 3123:3123 -e LOG_LEVEL=debug -e SORA_API_KEY=<YOUR_SORA_API_KEY> -e SORA_API_ENDPOINT=<YOUR_SORA_API_ENDPOINT> --gpus=all gyoridavid/short-video-maker:latest-cuda
```
...
services:
  short-video-maker:
    ...
    environment:
      ...
      - SORA_API_ENDPOINT=<YOUR_SORA_API_ENDPOINT>
    ports:
      - "3123:3123"
    volumes:
...
| ------------------- | -------------------------------------------------------------------------------- | ------- |
| SORA_API_KEY        | Your Azure Sora API Key.                                                         |         |
| SORA_API_ENDPOINT   | Your Azure Sora API Endpoint.                                                    |         |
...
## Can I select different source for the videos than Azure Sora, or provide my own video?

Currently, the integration is focused on Azure Sora for generative video. Providing your own video files directly is not supported through this generative pipeline.
...
| Azure Sora API                                         | N/A      | [Azure AI Services Terms](https://azure.microsoft.com/en-us/support/legal/cognitive-services-terms/) | Background video generation     |
...
- ❤️ Azure Sora (Microsoft) for generative video content
